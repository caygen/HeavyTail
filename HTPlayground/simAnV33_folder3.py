...
if PlotFitFast:
    ...
    axs.plot(xx_lin,BiexpFun(xx_lin,*Mcomp))
...
#%% Residual Comparison
fig, axs = plt.subplots(1, 1, figsize=(3.2, 3.0))
axs.plot(DataX,(DataY-function(DataX,*ret.x))/np.sqrt(DataY))
axs.plot(DataX,(DataY-function(DataX,*Mcomp))/np.sqrt(DataY))
# axs.plot(DataX,DataY - derFHTt(DataX,*ddtMihaelHT)*400/np.pi)
# axs.set_xlim(right=25000)
