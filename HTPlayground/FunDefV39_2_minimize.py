...
def PicoCompare(Data_t, 
    ...
    axs.plot(Data_t,PicoY,color='gray' ,label='PicoQuant', lw=1)
    axs.plot(Data_t, dHTy,color='darkred',label='HT Fit'   , lw=1)
...
def PicoIntCompare(Data_t, 
    ...
    axs.plot(   Data_t, IntDataExcelBack,color='lightcoral',linestyle = '-' , label='$\int$ Data - B$_{HT}$' ,lw = 5)
    axs.plot(   Data_t, IntDataPicoBack ,color='lightgray' ,linestyle = '--', label='$\int$ Data - B$_{PQ}$',lw = 3)
    
    axs.plot(   Data_t, HTy             ,color='darkred'   ,linestyle = '-' , label='  HT Fit'     ,lw = 0.7)
    axs.plot(   Data_t, IntPicoFitBack  ,color='gray'      ,linestyle = '-' , label='  PQ Fit'     ,lw = 0.7)
...
def PlotPicoComparisons(Data_t, DataOg, PicoY, 
    ...
    N = len(Data_t) # number of datapoints
    P = 4          # number of floating parameters
    
    Xi_PQ  = PQ_chiSquared(Data_t, PicoY, DataOg)
    XiR_PQ = Xi_PQ/(N-P)
    print( 'N : ' + str(N) + '; P :' + str(P))
    
    print( 'Xi^2 (PQ)   = ' + str(Xi_PQ) )
    print( 'Xi^2_R (PQ) = ' + str(XiR_PQ) )
    
    Xi_HT = PQ_chiSquared(Data_t, dHTy, DataOg)
    XiR_HT = Xi_HT/(N-P)
    
    print( 'Xi^2 (HT) = ' + str(Xi_HT) )
    print( 'Xi^2_R (HT) = ' + str(XiR_HT) )
    
    
#%% ChiSquared
def PQ_chiSquared(dataX,dataFit,dataOG,**kwargs):
...
def BiexpFun(t, T1, A1, T2, A2, B=0):
    ret = A1*np.exp(-t/T1) + A2*np.exp(-t/T2) + B
    return ret

def BiexpFun2(t, tau1, A, tau2, B):
    ret = B* [A*np.exp(-t/tau1) + (1-A)*np.exp(-t/tau2) ]
    return ret
...
def BiexpFunFixBLogA(t, T1, A1, T2, A2):#, B = 666):
    ...
    ret = np.log(A1)*(-(t)/(T1)) + np.log(A2)*np.exp(-(t)/(T2)) + np.log(2.4)
    
    return ret

#%% Derivative Heavy Tail Function
def derFHTt(t,T,B,df0,m):
    # ret = -df0*(B*(1-m)**2)/T * ((t/T)**(B-1)*np.exp((1-m)*(t/T)**B))/(np.exp((1-m)*(t/T)**B)-m)**2
    ret = -df0*(1-m)**2 * np.exp((1-m)*(t/T)**B) * B * (t/T)**(B-1) / (T *(np.exp((1-m)*(t/T)**B)-m)**2)
    return ret
#%%
...
def MakeResultsTable(DataX,DataY,function,ret,r,y_unit,title, **kwargs):
    ...
    mVal = '{0:.{1}f}'.format(ret.x[3]+1       ,sigfigs[3])
    m = ret.x[3]+1
    # else:
        # m_hi, m_lo = 9,9
    ...
    if abs(m_hi)>2: m_hi = 2
    if abs(m_lo)<1: m_lo = 1
    ...
    tVal = '{0}'.format(round_to_n(np.exp(ret.x[0])*tScale,5))
    ...
    tHiLo = '{0}'.format(round_to_n(t_hi*tScale,5)),'{0}'.format(round_to_n(t_lo*tScale,5))
...
def ChiSquaredCost(dataX,dataY,fun,fitParam,**kwargs):

    N = dataY.size-2
    # pdb.set_trace()
    ydata = np.array(dataY)
    yfit  = fun(dataX,*fitParam,**kwargs)
    
    summ = 0
    ii = 0
    
    summ = np.trapz((ydata-yfit)**2/abs(yfit), x=dataX, axis=-1)
    # while(ii<= N):
    #     summ += (ydata[ii]-yfit[ii])**2/yfit[ii]*(dataX[ii+1]-dataX[ii])
    #     ii += 1
    return summ/(dataX[-1]-dataX[0])

def ChiSquaredData(dataX,dataY,fitX,fitY,**kwargs):
    N = dataY.size-2
    # breakpoint()
    ydata = np.array(dataY)
    ...
    summ = np.trapz((ydata-yfit)**2/yfit, x=dataX, axis=-1)
...
def residulaChiSq(p, x, y, fun):
    return ChiSquaredCost(x,y,fun,p)
# #%%
...
#     accept = -1e4 #probability of acceptance: The lower the smaller the probability of acceptance. Default value is -5.0 with a range (-1e4, -5].
...
#                             accept = accept)
#                            # callback=callbackF,
#                            # no_local_search = False,visit = 1.01)
