...
functions = {
    "Biexponential": "f(t) = beta + A1 * e^(-t/tau1) + A2 * e^(-t/tau2)",  # Formula for biexponential function
    "Monoexponential": "f(t) = f0 + A * e^(-t/tau)",  # Formula for monoexponential function
    ...
    if file_path: #If user selected a file
        try:
            ...
            if 't' in data.columns and 'y' in data.columns:  # Check if file contains 't' and 'y' columns
                ...
                # Plot the raw data
                ax.clear()
                ax.plot(data['t'], data['y'], 'o', label='Data')
                ax.set_xlabel('t')
                ax.set_ylabel('y')
                ax.legend()
                canvas.draw()
...
def update_function_form_and_parameter_boundaries(*args): #*args: function is a callback function from function_var.trace
    ...
    parameter_labels = {
        "Biexponential": ["beta", "A1", "tau1", "A2", "tau2"],
        "Monoexponential": ["f0", "A", "tau"],
