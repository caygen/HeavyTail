...
#Choose a range in the raw data to fit the linear function to
start = 150000 #ns
# end =  #ns/

# raw_range = raw[(raw['Time [ns]'] > start) & (raw['Time [ns]'] < end)]
raw_range = raw[(raw['Time [ns]'] > start)]

#%%
# Apply Savitzky-Golay filter
window_size = 101  # Window size (must be odd)
poly_order = 1   # Polynomial order
smoothed_data = savgol_filter(raw['Intensity [Cnts.]'], window_size, poly_order)

# Use curve_fit to fit the linear function to the data
params, covariance = curve_fit(linearBackground, raw_range['Time [ns]'], raw_range['Intensity [Cnts.]'])

# Extract the slope (m) and intercept (b) from the fitting parameters
m_fit, b_fit = params

#%%
plt.plot(raw['Time [ns]'], raw['Intensity [Cnts.]'], label='Data')
plt.plot(DataX, DataY, label='Data')
plt.plot(raw['Time [ns]'], smoothed_data, label='Smoothed Data')

plt.plot(raw_range['Time [ns]'], linearBackground(raw_range['Time [ns]'], m_fit, b_fit), label='Fit')
...
DataYsubB = DataY - linearBackground(DataX, m_fit, b_fit)
DataYIntegraded = np.cumsum(DataYsubB)

plt.plot(DataX, DataYIntegraded)
plt.yscale('log')
# plt.xscale('log')
plt.xlabel('t [ns]')
plt.ylabel('Integrated Intensity [Cnts.]')
plt.grid()
plt.show()
...
Mcomp = Mcomp[:-1]
# funBounds = [TBounds, ABounds, TBounds, ABounds, BBounds]
# funBounds = [TBounds, ABounds, TBounds, ABounds]#, BBounds]


#%% FIT HERE
funBounds = [(1,1000), (0,1), (-1e3,3e5), (0,1)]
ret = heavyTailFit(x=DataX, y=DataYIntegraded/1e6, fun=function, pBounds=funBounds, maxiterations=1000)
